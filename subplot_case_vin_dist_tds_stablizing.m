...
sim_path_case = '..\data\simulation_result\Stablizing_TDS_Case_Vin_Dist_tau';
...
% close all;
% tstart = -tau; tend = Tf;
ctrl_Name = [{'Stablizing'},{'Controller'}];
...
lgd = legend(['\tau =  ',num2str(tau1*1e3),'   (ms)'],...
             ...
             ['V_',num2str(i),'ref:'], 'Location','northeast');
title(lgd,ctrl_Name);          
...
ax.YLim = [355, 430];
ax.YTick = 350:10:429;
ax.XLabel.String = {'Time (s)'; '(a)'};
ax.YLabel.String = 'Voltage (V)';


% subplot(2,2,4);
% plot(t_ta1, Vini_ta1, t_ta1,Vin(i)*ones(size(t_ta1)),'--','LineWidth',2);
% 
% lgd = legend('Disturbed', 'Nominal' ,'Location','northeast');
% title(lgd,['Vin_', num2str(i)]);
% 
% % Plot Settings
% grid on;
% ax = gca;
