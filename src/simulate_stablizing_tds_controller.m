...
%---------------------------------------------------------------------------------------------
% Simulation for Stablizing Controller

%% ---- Simulation time production + delay
% Closed loop system simulation (small signal)
set_current_path;
...
%% ---- Vin Disturbances
load('..\data\vin_signal\VinSignals_1min_10e5');
...
B = blkdiag(B1,B2,B3,B4,B5,B6);
    ...
    % numerical solution of system dynamics by Euler method
    delay = Nd; %floor(ta(indx_t-Nd)/dt);
     y(:,indx_t) = C*x(:,indx_t);
        
    u(:,indx_t) = K*y(:,indx_t);
%     x(:,indx_t+1)=x(:,indx_t) + (A*x(:,indx_t) + B*u(:,indx_t-Nd) + D*w(:,indx_t))*dt ;
...
u = [u, u(:,end)];  % Dimension adjustment
...
if sim_vin_dist
    ...
    save(File_Name, 'x','t','u','Dd','RL','Vin',...
                    ...
                    'epsilon','d','tau','h','dt','Tf', 'Elapsed_time_sec'); 
end
...
if sim_load_dist
    ...
    save(File_Name, 'x','t','u','Dd','RL','Vin',...
                    'w','w1','w2','w3','w4','w5','w6','Vref',...
                    'rl1','rl2','rl3','rl4','rl5','rl6',...
                    'epsilon','d','tau','h','dt','Tf','K', 'Elapsed_time_sec'); 
end
